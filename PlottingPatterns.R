...
file_names <- dir("Scans/Station_1-2021/Station_1/")
your_data_frame <- as_tibble(do.call(rbind,lapply(paste("Scans/Station_1-2021/Station_1/",file_names, sep=""),read.csv)))  # hit error with read_csv
...
df2 <- your_data_frame %>%
  ...
  filter((Time >= c("18:00:00") & Time <= c("23:59:58")) |
           (Time <= c("06:00:00") & Time >= c("00:00:00"))) %>%
  group_by(dt=cut(Time, breaks= seq(
    from=c("18:00:00"),
    to=c("06:00:00"),
    by="1"
  ))) 
...
p1 <- ggplot(dfsum, aes(x = Time, y = n, color = n, fill = n)) +
  ...
  ylab("Scan Count") +
  scale_x_discrete(labels = labs) +
  theme(legend.position = "none", panel.background = element_rect(fill = "white", color = "darkgrey"), axis.title = element_text(size = 55), axis.text = element_text(size = 50))

png(file="ScanCounts.png",width=5,height=5,units="in",res=600)
p1
dev.off()
